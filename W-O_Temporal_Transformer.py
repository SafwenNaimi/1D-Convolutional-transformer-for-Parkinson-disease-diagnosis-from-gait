...
"""
#Here, we chose a fixed one with constant step in between the different positional encoding.
...
def stack_block_transformer(num_transformer_blocks):
    input1 = Input(shape=(22, 1))   #keras.Input(shape=(100, 1))
    x = input1
    x = CNN1D(x)
    for _ in range(num_transformer_blocks):
        x = transformer_encoder(x,22,2)
    x = layers.GlobalAveragePooling1D(data_format="channels_first")(x)
    x = layers.Dense(10, activation='selu')(x)
    return input1,x
def stack_block_transformer_spatial(num_transformer_blocks,x):
  for _ in range(num_transformer_blocks):
      x = transformer_encoder(x,10*18,2)# TWO ENCODERS BLOCKS
  x = layers.GlobalAveragePooling1D(data_format="channels_first")(x)
  return x
...
####
  # initialise with the first input
    ...
    model = Model(inputs, answer)
    opt = optimizers.RMSprop(lr=0.001)
    model.compile(loss='binary_crossentropy', optimizer=opt, metrics=['accuracy'],experimental_run_tf_function=False) ##It was binary_crossentropy
    print(model.summary())
    return model
model = multiple_transformer_5_level(18)
model.summary
"""
...
def transformer_encoder(inputs,key_dim,num_heads):
    ...
    res = x + inputs
    # Feed Forward Part
    x = layers.Conv1D(filters=4, kernel_size=1, activation="selu")(res)
    x = layers.Dropout(dropout)(x)
...
def train( model, datas, lr, log_filename, filename):
    ...
    for i in (np.arange(1,4)*5):  # 10-20    1-10
        ...
        callbacks_list = [checkpointer, early_stopping, logger]#, tf.keras.callbacks.LearningRateScheduler(lr_scheduler)]
        #print("Y_train")
        #print(len(y_train))
